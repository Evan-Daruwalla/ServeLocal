...
These assume managed hosting (Railway/equivalent) with a **persistent volume** for `db.json`
...
|---|---|---|
| Corrupt `db.json` | Boot log + `/api/health/ready` shows `db: degraded` | On boot the app auto-restores the newest valid backup, else reseeds, then **rewrites** a healthy `db.json` |
...
node scripts/backup.js          # -> backups/db-<timestamp>.json  (keeps newest 48)

# Restore the newest valid snapshot (snapshots current state first for safety)
node scripts/restore.js --force # overwrites db.json, then app loads it on next boot
```
