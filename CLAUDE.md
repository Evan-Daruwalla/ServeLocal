...
- **Indexes (ADR-0011)**: hot lookups go through `IDX()` — Maps keyed by id (`userById`/`oppById`/`appById`/`hoursById`), foreign key (`appsByOpp`/`appsByUser`/`hoursByUser`/`notifsByUser`/`messagesByOpp`/`reviewsByOrg`/`oppsByOrg`/`endorsementsByUser`), plus `userByEmail`/`userByOrgId`. Derived state, rebuilt lazily when the `DB` ref changes or `_cacheVersion` bumps (same invalidation as the read cache), so never stale past the last `saveDB()`. Use `IDX().xById.get(id)`, `ownedOpp(id,orgId)` for org-owned lookups, and `idxList(map,key)` for FK groups. **Never mutate the array `idxList` returns** (shared bucket / frozen empty) — spread first. `findIndex` write-path sites stay as scans (they need the array position). Covered by `test/index.test.js`.
