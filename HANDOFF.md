...
| **Graph-audit batch + CSP hardening (2026-07-05)** | **Done** | Six items from a `/graphify` audit: (1) fixed stale `docs/security.md`; (2) notification email delivery + per-user opt-out toggle; (3) zero-dep TOTP MFA (`lib/totp.js`, `test/mfa.test.js`); (4) extracted SPA JS to `public/app.js` (ADR-0014 step 1), then converted all ~271 inline handlers to a delegated dispatch table and **dropped `'unsafe-inline'` from `script-src`** (step 2); (5) split `server.js` → `lib/persist.js` + `lib/auth.js`, ADR-0015. 74 tests, chaos 3/3, preview verified. Committed `e092dea` (not pushed) |
